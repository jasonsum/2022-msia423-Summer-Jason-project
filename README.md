...
The selected model is served to users through an open web application, wherein users can select various health outcomes, population sizes, and geographical areas to receive a predicted population proportion with poor/fair health. More importantly, the application seeks to accomplish the two key tasks, previously mentioned. First, it highlights the impacts in predicted population proportions based on changing user inputs. Secondly, it provides a proactive tool for users to identify populations that may be at-risk based on trending figures. 
